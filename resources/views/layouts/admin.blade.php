...
<aside class="admin-sidebar"><a class="admin-brand" href="{{ route('admin.dashboard') }}">EPIM Admin</a>@foreach(['formations'=>'Formations','actualites'=>'Actualités','galerie'=>'Galerie','inscriptions'=>'Inscriptions','messages'=>'Messages','partenaires'=>'Partenaires','pages'=>'Pages / SEO','rubriques'=>'Rubriques','parametres'=>'Paramètres','utilisateurs'=>'Utilisateurs'] as $key=>$label)<a href="{{ route('admin.resource',$key) }}"><i class="bi bi-grid"></i> {{ $label }}</a>@endforeach<form method="post" action="{{ route('logout') }}">@csrf<button><i class="bi bi-box-arrow-right"></i> Déconnexion</button></form></aside>
